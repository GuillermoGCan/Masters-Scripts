...
function(x, y, box.init=NULL, peel.alpha=0.05, paste.alpha=0.01,
                 ...
                 threshold.type=1, paste.all=FALSE,coverage=TRUE,
								 showbounds=TRUE,style="ineq", npts=NA,
								 ninter=NA,nbump=10,repro=TRUE,dfrac=.5,
...
options(digits=4)
  ...
  if (is.null(box.init))
  {
    ...
    box.diff <- box.init[2,] - box.init[1,]
		#Bryant edit starting after version 2.32-1:
		#Never clear why Duong added a "10" multiplier to the box bounds, 
		#Doing so makes alternate peeling criteria not work right.
		#Changing it to a 1:
		#Now changing to a zero by commenting out:
    box.init[1,] <- box.init[1,] #- 1*paste.alpha*box.diff
    box.init[2,] <- box.init[2,] #+ 1*paste.alpha*box.diff
  }

  ## find first box
  #k <- 1
	#print("calling find.traj")
  flush.console()
	boxseq <- find.traj(x=x, y=y, box=box.init, peel.alpha=peel.alpha,
                   paste.alpha=paste.alpha, mass.min=mass.min,
                   threshold=min(y)-0.1*abs(min(y)), d=d, n=n, pasting=pasting, 
									 verbose=verbose, paste.all=paste.all,
									 peel_crit=peel_crit)
  ...
  while(!satisfied){
    ...
    checpts <- trajplot(trajinf, coverage=coverage, margtrajs=margtrajs, 
...
######      #Section for automatically assessing reproducibility
      if(repro){ #TRUE=test reproducibility stats
        ...
        for (b in 1:nbump){
          ...
          boxseqlist[[b]] <- find.traj(x=x[curs,], y=y[curs], box=box.init, 
											peel.alpha=peel.alpha,
											paste.alpha=paste.alpha, mass.min=mass.min,                  
											threshold=min(y)-0.1*abs(min(y)), d=d, n=n, 
											pasting=pasting, verbose=verbose, paste.all=paste.all,
											peel_crit=peel_crit)
...
#          osdimsout <- curmats[[2]][remrs,4] #os = onesided - then need to figure out whether top or bottom
          ...
          ldimsout <-  curmats[[2]][remrs,4][curmats[[2]][remrs,2]==" > "]
          udimsout <-  curmats[[2]][remrs,4][curmats[[2]][remrs,2]==" < "]
